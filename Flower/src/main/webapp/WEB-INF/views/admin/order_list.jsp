	...
	<table class="table table-bordered table-hover">
		...
		<c:forEach items="${p.list}" var="order">
			<tr>
				...
				<td>
					<p>
						<c:if test="${order.status==1}"><span style="color:blue;font-weight:bold;">未付款</span></c:if>
						<c:if test="${order.status==2}"><span style="color:red;font-weight:bold;">已付款</span></c:if>
						<c:if test="${order.status==3}"><span style="color:green;font-weight:bold;">已发货</span></c:if>
						<c:if test="${order.status==4}"><span style="color:black;font-weight:bold;">已完成</span></c:if>
					</p>
				</td>
				<td>
					<p>
