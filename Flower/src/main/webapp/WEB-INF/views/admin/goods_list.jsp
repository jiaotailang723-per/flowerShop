...
<div class="container-fluid">
	...
	<div class="text-right"><a class="btn btn-warning" href="${pageContext.request.contextPath}/admin/goods_add">添加商品</a></div>
	<br>
	...
	<table class="table table-bordered table-hover">
		...
		<c:forEach items="${p.list}" var="g">
			<tr>
				<td><p>${g.id}</p></td>
				<td><p><a href="${pageContext.request.contextPath}/flowers_detail?id=${g.id}" target="_blank"><img src="${pageContext.request.contextPath}${g.cover}" width="100px" height="100px"></a></p></td>
				<td><p><a href="${pageContext.request.contextPath}/flowers_detail?id=${g.id}" target="_blank">${g.name}</a></p></td>
				<td><p>${g.intro}</p></td>
				<td><p>${g.price}</p></td>
				...
				<td>
					...
					<a class="btn btn-success"
					   href="${pageContext.request.contextPath}/admin/goods_editshow?id=${g.id}&pageNumber=${p.pageNumber}&type=${type}">
						修改
					</a>
					<a class="btn btn-danger"
					   onclick="return confirm('确定要删除该商品吗？')"
					   href="${pageContext.request.contextPath}/admin/goods_delete?id=${g.id}&pageNumber=${p.pageNumber}&type=${type}">
						删除
					</a>
				</td>
			</tr>
